...
{
  LOG_WT("*** SERVER 1 - tran xuan hien ***\n");
  ...
  // Nếu dữ liệu được nhận thành công, hãy ghi nhật nó vào bảng điều khiển
  if (iResult > 0) {
    // Thêm ký tự null kết thúc chuỗi
    recvbuf[iResult] = 0;
    LOG_D("So byte nhan duoc: %d - noi dung\n%s\n", iResult, recvbuf);

    // Kiểm tra xem dữ liệu có phải là chuỗi "exit" hay không
...
} while (iResult > 0); // Vòng lặp sẽ tiếp tục cho đến khi không còn dữ liệu nào để nhận hoặc kết nối bị đóng
